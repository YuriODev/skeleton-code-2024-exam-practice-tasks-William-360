...
########################################task 6##############################################
    def SavePuzzle(self,Filename):
        with open(Filename, "a") as f:
            print(len(self.__AllowedSymbols), file = f)
            for S in self.__AllowedSymbols:
                print(S, file = f)
            print(len(self.__AllowedPatterns), file = f)
            for P in self.__AllowedPatterns:
                f.write(P.GetPatternSymbol())
                f.write(",")
                print(P.GetPatternSequence(), file = f)
            print(self.__GridSize, file = f)
            for C in self.__Grid:
                ...
                if len(NotAllowedSymbols) == 0:
                  print(",", file = f)
                else:
                    for S in NotAllowedSymbols:
                        f.write(",")
                        if NotAllowedSymbols.index(S) == len(NotAllowedSymbols) - 1:
                            print(S, file = f)
                        else:
                            f.write(S)
            print(self.__Score, file = f)
            print(self.__SymbolsLeft, file = f)
...
#####################################task 5#############################################
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Remove = input("Do you want to remove a symbol?: ").lower()
            Row = -1
            Valid = False
            while not Valid:
                try:
            ...
            if Remove != "y":
                ...
                if CurrentCell.IsEmpty():
                    print("You can't change an empty cell.")
                    self.__SymbolsLeft -= 1

                elif not(CurrentCell.CheckSymbolAllowed(CurrentCell.GetSymbol())):
                    print("Cell symbol cannot be changed")
                    self.__SymbolsLeft -= 1  
...
############################task 6############################                
            Save = input("Do you want to save the puzzle?: ").lower()
            if Save == "y":
                Filename = input("Enter filename: ")
                self.SavePuzzle(Filename + "txt")
